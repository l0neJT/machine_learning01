...
# Append column for 'ABE' versuse 'CD' classes
train$split.classe <- factor(train$classe %in% c("C", "D"), labels = c("ABE", "CD"))

# Model split between 'ABE' and 'CD'
splitModel <- train(train[, -c(1, 3:7, 60)], train[, 61], method = "rpart", trControl = tr)
splitPred <- predict(splitModel, train[, -c(1, 3:7, 60)])
print(confusionMatrix(splitPred, train[, 61]))

# Subset and model 'ABE' only
abeTrain <- subset(train, split.classe == "ABE")
abeTrain <- transform(abeTrain, classe = factor(abeTrain$classe))
abeModel <- train(abeTrain[, -c(1, 3:7, 61)], abeTrain[, 60], method = "rpart", trControl = tr)
abePred <- predict(abeModel, abeTrain[, -c(1, 3:7, 61)])
print(confusionMatrix(abePred, abeTrain[, 60]))

# Subset and model 'CD' only
cdTrain <- subset(train, split.classe == "CD")
cdTrain <- transform(cdTrain, classe = factor(cdTrain$classe))
cdModel <- train(cdTrain[, -c(1, 3:7, 61)], cdTrain[, 60], method = "rpart", trControl = tr)
cdPred <- predict(cdModel, cdTrain[, -c(1, 3:7, 61)])
print(confusionMatrix(cdPred, cdTrain[, 60]))
```
